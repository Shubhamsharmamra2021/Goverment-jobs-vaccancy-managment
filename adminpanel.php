...
<body>
  ...
  <form action="<?php echo htmlentities($_server['PHP SELF']); ?>" method="POST">
  	<h3>Admin Login</h3>
  ...
  if (isset($_POST['submit'])) {
   ...
   if ($query) {
   	  ...
   	  if (mysqli_num_rows($query)>0) {
                      $emailp= mysqli_fetch_assoc($query);
        $_SESSION['username'] = $emailp['name'];
   	  	  header('location:inresult.php');
      }else{
